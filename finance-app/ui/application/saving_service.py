...
class saving_service:
    ...
    def total_saving(self):
        ...
        return self.saving_df["balance"].sum()

    def calculate_saving_balance(self):
        # Group by year and month
        self.saving_df["YearMonth"] = self.saving_df["transaction_date"].dt.to_period(
            "M"
        )

        monthly_balance = (
            self.saving_df.groupby("YearMonth")["balance"].sum().reset_index()
        )
